...
import FloodGI from './FloodGI/FloodGI';



export default function FloodHeader({ ResultsPerPageDrop }) {

    const [PerPage, setPerPage] = useState("FloodGI");
    ...
    return (
        <div>
                ...
                <Box flex="1">
                    <Center>
                        <HStack justifyContent='center' boxShadow='2xl' style={{ justifyContent: "space-around" }} width='90%' height='10vh' align='center' rounded='md' mt="4">
                            ...
                            {/* <Box boxShadow='md'> */}
                            <Text onClick={() => setPerPage('FloodGI')} cursor="pointer" _hover={{ textDecoration: "underLine" }}>Trucking</Text>
...

            {
                ...
                PerPage === 'FloodGI' ? <><FloodGI /></> :
                    PerPage === 'Premiums' ? <><FloodGI /></> :
                        PerPage === 'Nodes' ? <><Nodes /></> :
                            PerPage === 'Diary' ? <><Diary /></> :
                                PerPage === 'Memos' ? <><Memos /></> :
                                    <></>}
